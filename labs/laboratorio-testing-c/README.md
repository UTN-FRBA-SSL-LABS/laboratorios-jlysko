...
> R:inicializa la cantidad del carrito en cero, es importante llamarla porque sino se estaria usando el carrito sin inicializar
...
> R:linea 24, suma unicamente el precio de cada producto sin tener en cuenta la cantidad

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:tener en cuenta tanto precio como cantidad de cada producto
...
> R:el operador incorrecto es el <=, deberia ser <
...
> R:si, la linea 29 no se ejecuto porque nunca se aplica descuento en test_unitarios
...
> R:un test unitario testea cada funcionalidad por separado, los de integreacion las prueban en conjunto. el bug de carrito_total lo detecto primero el unitario
...
> R:en C el usuario se encarga de el manejo de memoria dinamica, en cambio en Python o Java se encarga el administrador de memoria del lenguaje automaticamente
...
> R:mirando el codigo se te pueden pasar cosas por alto, en cambio haciendo los test adecuados se pueden cubrir todos los casos
...
> R:porque como la cantidad era 1, no hace falta multiplicar por 1 porque da el mismo resultado
